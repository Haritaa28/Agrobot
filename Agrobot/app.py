...
from gemini_helper import analyze_with_gemini  # if used elsewhere
...
def api_chat():
    try:
        ...
        # KB + multilingual logic inside process_message
        reply = process_message(user_profile, message)

        # Fallback: try gemini/ask_gemini if reply empty
        if not reply or not reply.strip():
            try:
                from gemini_helper import ask_gemini
                reply = ask_gemini(message)
            except Exception as e:
                print("Gemini fallback not available:", e)
                reply = ""
...
def admin_upload_kb_csv():
    ...
    # CSV parse supporting multilingual columns: keywords,answer_en,answer_hi,answer_ta,answer_ml,answer_te
    import csv
    rows = []
    try:
        with open(path,newline='',encoding='utf-8') as csvfile:
            ...
            for r in reader:
                ...
                rows.append({
                    ...
                    "answer_ml": r.get('answer_ml') or "",  # added
                    "answer_te": r.get('answer_te') or ""   # added
                })

        # merge into KB file (preserve existing entries)
        try:
            with open(KB_PATH,"r",encoding='utf-8') as f:
        ...
        # reload KB in memory if your chatbot_model supports it
        try:
            load_kb()  # if load_kb updates a global KB, it will refresh
        except Exception:
            pass
...
def analyze_image():
    ...
    try:
        ...
        # Analyze image (simple heuristic)
...
if __name__ == "__main__":
    # When deploying, set debug=False and use a production WSGI server
